...
impl SweepModel {
    ...
    /// Automatically configures the sweep model based on the known devices in the system.
    ...
    /// Add the Step Channel settings to the val_replacement_map for script generation.
    ...
    /// Add the Sweep Channel settings to the val_replacement_map for script generation.
    ...
    /// Add the Bias Channel settings to the val_replacement_map for script generation.
    ...
    /// Add the common settings to the val_replacement_map for script generation.
    fn define_common_settings(&mut self) {
        ...
        // Add the global values to the value replacement map ... any device should be suitable for
    ...
    /// Deduce the master node of this function chunk. This is the node with the step channel. If there
    /// isn't a step channel, it is the node of the first sweep channel.
    ///
    /// # Returns
    ///
    /// A string representing the master node ID.
    ...
    /// Build a list of SMUs that are in channels and associated with a node in list. The format
    /// of the list is nested ICL tables (e.g., {smu1,smu2},{smu3},{smu4,smu5}).
    /// The order of this list must match the order generated by format_pulse().
    ///
    /// # Arguments
    ///
    /// * `list` - A reference to a vector of node IDs.
    /// * `available` - A reference to a vector of available `SweepChannel` instances.
    /// * `prepend_comma` - A boolean indicating whether to prepend a comma.
    ///
    /// # Returns
    ///
    /// A formatted string representing the SMUs.
    ...
    /// Build a list of pulse setting that are in channels and associated with a node in list.
    /// The format of the list is nested ICL tables (e.g., {true,false},{false},{true,true}).
    /// The order of this list must match the order generated by format_smus()
    ///
    /// # Arguments
    ///
    /// * `list` - A reference to a vector of node IDs.
    ...
    /// # Returns
    ///
    /// A formatted string representing the pulse settings.
...
/// Represents attributes for configuring the sweep model.
#[derive(Debug)]
